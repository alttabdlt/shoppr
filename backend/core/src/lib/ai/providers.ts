...
export const myProvider = isTestEnvironment
  ...
  : (() => {
      const openrouter = createOpenRouter({
        // Prefer OpenRouter key, fall back to AI Gateway style var
        apiKey: process.env.OPENROUTER_API_KEY || process.env.AI_GATEWAY_API_KEY,
        // Allow overriding the base URL (e.g., via a gateway proxy)
        baseURL:
          process.env.OPENROUTER_BASE_URL || process.env.AI_GATEWAY_BASE_URL,
