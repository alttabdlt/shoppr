...
import { Suggestion as PreviewSuggestion } from '@/features/chat/elements/suggestion';
...
function findPositionsInDoc(doc: Node, searchText: string): Position | null {
  ...
  doc.nodesBetween(0, doc.content.size, (node, pos) => {
    if (node.isText && node.text) {
      ...
      if (index !== -1) {
        positions = { start: pos + index, end: pos + index + searchText.length };
        return false;
      }
    }
    return true;
...
export function projectWithPositions(doc: Node, suggestions: Array<Suggestion>): Array<UISuggestion> {
  return suggestions.map((suggestion) => {
    ...
    if (!positions) {
      return { ...suggestion, selectionStart: 0, selectionEnd: 0 } as UISuggestion;
    }
    return { ...suggestion, selectionStart: positions.start, selectionEnd: positions.end } as UISuggestion;
  });
}
...
export function createSuggestionWidget(
  ...
  return { dom, destroy: () => setTimeout(() => root.unmount(), 0) };
}
...
export const suggestionsPlugin = new Plugin({
  ...
  state: {
    init() { return { decorations: DecorationSet.empty, selected: null }; },
    apply(tr, state) {
      const newDecorations = tr.getMeta(suggestionsPluginKey);
      if (newDecorations) return newDecorations;
      return { decorations: state.decorations.map(tr.mapping, tr.doc), selected: state.selected };
    },
  },
