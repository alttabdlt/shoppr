/*
  ...
  Usage: npx -y tsx scripts/test-mcp.ts
*/
...
async function main() {
  ...
  const { initializeToolRegistry, getToolRegistry } = await import('../packages/core/src/lib/tools');
  const { registerMCPServersFromConfig } = await import('../packages/core/src/lib/mcp-servers');
